                ...
                return (
                  <div key={companion.id} className="border border-[var(--border)] bg-[var(--surface)] p-4">
                    <div className="flex flex-wrap items-center gap-2">
                      <Link href={`/reference/companions/${companion.id}`} className="text-sm font-medium text-black hover:text-black/84">
                        {companion.name}
                      </Link>
                      <Badge variant="teal">Rank #{recommendation?.rank}</Badge>
                      <Badge variant="purple">{companion.role_tag}</Badge>
                    </div>
                    <p className="mt-2 text-sm leading-6 text-black/82">{recommendation?.benefit}</p>
              ...
              <div className="grid gap-3">
                {companionPowerSnapshots.slice(0, 48).map((item) => (
                  <div key={item.name} className="border border-[var(--border)] bg-[var(--surface)] p-4">
                    ...
                    <Link href={`/reference/companion-powers/${item.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "")}`} className="mt-3 block text-sm font-medium text-black hover:text-black/84">
                      {item.name}
                    </Link>
              ...
              <div className="grid gap-3">
                {companionEnhancementSnapshots.map((item) => {
                  ...
                  return (
                    <div key={item.name} className="border border-[var(--border)] bg-[var(--surface)] p-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <Link href={`/reference/enhancements/${item.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "")}`} className="text-sm font-medium text-black hover:text-black/84">
                          {item.name}
                        </Link>
